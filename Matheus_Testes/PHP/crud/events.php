...
if ($method === 'POST') {
  ...
  if ($action === 'create') {
    ...
    $host = $_POST['host_user_id'] ?? null;
    $collection = $_POST['collection_id'] ?? null;
    $stmt = $mysqli->prepare('UPDATE events SET name=?, localization=?, event_date=?, type=?, summary=?, description=?, host_user_id=?, collection_id=? WHERE event_id=?');
