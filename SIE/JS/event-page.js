...
(function initCalendar() {
  ...
  if (!calendarView || !calendarGrid || !calendarMonthYear || !calendarToggleBtn || !calendarToggleText || !calendarPrevBtn || !calendarTodayBtn || !calendarNextBtn) {
    return;
  }

  const dataEl = document.getElementById('event-page-data');
  let payload = {};
  if (dataEl) {
    try {
  ...
  const eventsForCalendar = Array.isArray(payload.eventsForCalendar) ? payload.eventsForCalendar : [];
  const canCreateEvents = !!payload.isAuth;
  const createAllowedFromDate = payload.createAllowedFromDate || '';
  const eventsFormUrl = payload.eventsFormUrl || 'events_form.php';

  const monthsPT = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'];
  const daysPT = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'];

  let currentDate = new Date();
  let allEvents = eventsForCalendar.slice();
