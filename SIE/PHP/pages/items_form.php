    ...
    <form class="form-card" action="items_action.php" method="POST" enctype="multipart/form-data">
      ...
      <div style="background:#f8fafc; padding:16px; border-radius:14px; border:1px solid #e5e7eb; box-shadow: inset 0 1px 0 #f1f5f9;">
        <?php foreach ($ownedCollections as $col): ?>
          <?php
            $colId   = $col['id'];
            $checked = in_array($colId, $existingCollections, true);
          ?>
          <label style="display:flex; align-items:center; gap:10px; padding:8px 0; border-bottom:1px solid #e5e7eb; font-weight:600; color:#1f2937;">
            <input type="checkbox"
                   name="collectionIds[]"
                   value="<?php echo htmlspecialchars($colId); ?>"
