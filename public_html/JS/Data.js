...
const collectionsData = {
  ...
  userFollows: {
    // Simulated relationships in lieu of a real backend.
    "collector-main": ["rui-frio"],
    "rui-frio": ["collector-main"]
  },
  ...
  // ======================================================
  collectionRatings: [
    { collectionId: "escudos", userId: "collector-main", rating: 5 },
    { collectionId: "escudos", userId: "rui-frio", rating: 4 },
    { collectionId: "playboys", userId: "rui-frio", rating: 4 },
    { collectionId: "pokemon", userId: "collector-main", rating: 4 },
    { collectionId: "pokemon", userId: "rui-frio", rating: 5 },
    { collectionId: "jerseys", userId: "rui-frio", rating: 4 }
  ],
  // ======================================================
  // ITEM RATINGS
  // ======================================================
  itemRatings: [
    { itemId: "escudos-item-1", userId: "collector-main", rating: 5 },
    { itemId: "escudos-item-1", userId: "rui-frio", rating: 4 },
    { itemId: "escudos-item-2", userId: "collector-main", rating: 5 },
    { itemId: "playboys-item-2", userId: "rui-frio", rating: 4 },
...
(function(window) {
  // Load initial data from localStorage or use the default object
  let data = JSON.parse(localStorage.getItem("collectionsData")) || collectionsData;

  // If localStorage is empty, populate it with the default data
  ...
  function ensureUserFollowsMap() {
    if (!data.userFollows || typeof data.userFollows !== "object") {
      data.userFollows = {};
    }
  ...
  function findOrCreateUserShowcase(ownerId) {
    let userShowcase = data.userShowcases.find(us => us.ownerId === ownerId);
    if (!userShowcase) {
      userShowcase = {
        ownerId: ownerId,
        picks: [],
      ...
      data.userShowcases.push(userShowcase);
    }
    return userShowcase;
  }
  ...
  const appData = {
    loadData: function() {
      // Always return a deep copy to prevent direct mutation of the internal state
      return JSON.parse(JSON.stringify(data));
    },

    // Persists the like status for an item
    setUserItemLike: function(ownerId, itemId, isLiked) {
      if (!ownerId || !itemId) return;
      const userShowcase = findOrCreateUserShowcase(ownerId);
    ...
    // Persists the like status for an event
    setUserEventLike: function(ownerId, eventId, isLiked) {
    ...
    getUserFollowing: function(followerId) {
      if (!followerId) return [];
      const followsMap = ensureUserFollowsMap();
      const follows = followsMap[followerId];
      return Array.isArray(follows) ? [...follows] : [];
    },

    isUserFollowing: function(followerId, targetOwnerId) {
      if (!followerId || !targetOwnerId) return false;
      return this.getUserFollowing(followerId).includes(targetOwnerId);
    },

    toggleUserFollow: function(followerId, targetOwnerId) {
  ...
  // Expose appData to the global scope
  window.appData = { ...window.appData, ...appData };
